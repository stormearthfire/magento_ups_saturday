...
class Codernia_UpsSaturday_Model_Ups extends Mage_Usa_Model_Shipping_Carrier_Ups
{
        ...
        $codes = array(
            ...
            'originShipment'=>array(
                ...
                'United States Domestic Shipments' => array(
                    ...
                    '91' => Mage::helper('usa')->__('UPS Next Day Air Saturday Delivery'),
                    '104' => Mage::helper('usa')->__('UPS Next Day Air Early A.M. Saturday Delivery'),
                    '92' => Mage::helper('usa')->__('UPS Second Day Air Saturday Delivery'),
                ),
                    ...
                    'filters'    => array(
                        'within_us' => array(
                            'method' => array(
                                ...
                                '02', // 2nd Day Air
                                '91', // UPS Next Day Air Saturday
                                '92', // UPS Second Day Air Saturday
                                '104', // UPS Next Day Air Early A.M. Saturday
                            )
                        ),
                        'from_us' => array(
                            'method' => array(
...
XMLRequest;
        ...
        $result = $this->_parseXmlResponse($xmlResponse);

        if(Mage::getStoreConfig('carriers/ups/saturday_delivery')) {

            $saturday_marker = "<ShipmentServiceOptions><SaturdayDelivery /></ShipmentServiceOptions>";

            $xmlRequest = str_replace("</Shipment>", $saturday_marker . "</Shipment>", $xmlRequest) ;
            $xmlResponse = $this->_getCachedQuotes($xmlRequest);
            if ($xmlResponse === null) {
                $debugData = array('request' => $xmlRequest);
                try {
            ...
            $saturday_delivery = $this->_parseXmlResponse($xmlResponse, 1);
            foreach($saturday_delivery->getAllRates() as $sd) {
                $result->append($sd);
            }
     ...
     * @return Mage_Shipping_Model_Rate_Result
     */
    protected function _parseXmlResponse($xmlResponse, $saturdayIndicator = null)
    {
        $costArr = array();
        $priceArr = array();
